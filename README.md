...
Firebase konfigurace je rozdělena na stage/prod:
- `frontend/src/environments/firebase.config.stage.ts`
- `frontend/src/environments/firebase.config.prod.ts`

Alias `frontend/src/environments/firebase.config.ts` ukazuje na stage konfiguraci pro lokální vývoj a preview buildy.
...
Pokud potřebuješ změnit projekt, uprav odpovídající stage/prod soubor a změny commitni do repozitáře.
...
cd frontend
npm install
# případné změny Firebase projektu proveď v src/environments/firebase.config.stage.ts
npm start
```
